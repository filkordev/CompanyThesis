...
export class ContactApiService{
    ...
    getContacts(): Observable<Contact[]>{
        ...
        return this.http.get('/contacts?token=' + token)
        .map(
            (res: Response) => {
    ...
    getContact(id: number): Observable<Contact>{
        ...
        return this.http.get('/contact/' + id + '?token=' + token)
        .map(
            (res: Response) => {
    ...
    addContact(contact: Contact): Observable<any>{
        ...
        return this.http.post('/contact?token=' + token, body, {headers: headers});
    }
    ...
    updateContact(contact: Contact): Observable<any>{
        ...
        const headers = new HttpHeaders({'Content-Type': 'application/json'});
        const body = this.contactToDatabase(contact);
        return this.http.put('/contact/' + contact.id + '?token=' + token, body, {headers: headers});
    }
    ...
    deleteContact(contact: Contact | number): Observable<any>{
        ...
        const token = this.authService.getToken();
        return this.http.delete('/contact/' + id + '?token=' + token);
    }
}